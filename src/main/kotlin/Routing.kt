...
fun Application.configureRouting() {
    ...
    val handphoneService: HandphoneService by inject()
    ...
    routing {
        get("/") {
            call.respondText("API telah berjalan. Dibuat oleh Maharani Sitorus.")
        }

        // ✅ Route Plants (dipertahankan)
        route("/plants") {
        ...
        // ✅ Route Handphones (baru)
        route("/handphones") {
            get { handphoneService.getAllHandphones(call) }
            post { handphoneService.createHandphone(call) }
            get("/{id}") { handphoneService.getHandphoneById(call) }
            put("/{id}") { handphoneService.updateHandphone(call) }
            delete("/{id}") { handphoneService.deleteHandphone(call) }
            get("/{id}/image") { handphoneService.getHandphoneImage(call) }
        }

        // ✅ Route Profile (dipertahankan)
        route("/profile") {
